...
def get_default_train_val_test_loader(args):
    ...
    # get dataset from .pt
    data_train  = torch.load(f'Mortality/X_train.pt')
    # data_val    = torch.load(f'Mortality/X_valid.pt')
    data_val    = torch.load(f'Mortality/X_test.pt')
    
    label_train = torch.load(f'Mortality/y_train.pt')
    # label_val   = torch.load(f'Mortality/y_valid.pt')
    label_val   = torch.load(f'Mortality/y_test.pt')
...
def get_default_test_loader(args):
    ...
    # data_train  = torch.load(f'Mortality/X_train.pt')
    # data_val    = torch.load(f'Mortality/X_valid.pt')
    data_test    = torch.load(f'Mortality/X_test.pt')
    
    # label_train = torch.load(f'Mortality/y_train.pt')
    # label_val   = torch.load(f'Mortality/y_valid.pt')
    label_test   = torch.load(f'Mortality/y_test.pt')

    # label_train = label_train.flatten().to(dtype=torch.int64)
    # label_val   = label_val.flatten().to(dtype=torch.int64)
    label_test   = label_test.flatten().to(dtype=torch.int64)
    # init [num_variables, seq_length, num_classes]
    num_nodes = data_test.size(-2)

    seq_length = data_test.size(-1)
    
    num_classes = len(torch.bincount(label_test.type(torch.int)))
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset,batch_size=args['val_batch_size'],shuffle=False,num_workers=args['workers'],pin_memory=True)
