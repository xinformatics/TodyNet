...
class ICUVariableLengthDataset(Dataset):
    ...
    def __init__(self, source_path, split='train', maxlen=-1, task='Mortality_At24Hours', scale_label=False):
        """
        Args:
        ...
        """
        self.h5_loader = ICUVariableLengthLoaderTables(source_path, batch_size=1, maxlen=maxlen, task=task, splits=[split])
        # self.h5_loader = ICUVariableLengthLoaderTables(source_path, batch_size=1, maxlen=maxlen, splits=[split],  use_feat=True)
    ...
    def __getitem__(self, idx):
        ...
        # data, label, pad_mask = self.h5_loader.sample(None, self.split, idx)
        # print(type(data))
...
class ICUVariableLengthLoaderTables(object):
    ...
    def __init__(self, data_path, on_RAM=True, shuffle=True, batch_size=1, splits=['train', 'val'], maxlen=-1, task=0,
                 data_resampling=1, label_resampling=1, use_feat=False): #use_feat=False
        """
        Args:
        ...
        # print([name.decode('utf-8') for name in self.data_h5['labels']['tasks'][:]])
        self.splits = splits
        self.maxlen = maxlen
